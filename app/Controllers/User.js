									...
									try {
										UserInfo.create({'id':client.UID, 'name':name, 'joinedOn':new Date()}, function(errC, user){
											if (!!errC) {
												...
												Message.create({'uid': client.UID, 'title':'Thành Viên Mới', 'text':'Chào mừng bạn đến với Soul68, Chúc bạn chơi game vui vẻ...!', 'time':new Date()});
												client.redT.telegram.sendMessage(idNumbertele, name + " : Người dùng vừa tạo tài khoản mới:" , {parse_mode:'markdown', reply_markup:{remove_keyboard: true}});
												GameState(client);
												client.red(data);
											}
										});
